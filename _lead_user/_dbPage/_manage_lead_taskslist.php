...
$uid_lead_project = uniqid();
$uid_task = uniqid();


if($isNew === 'true')
{
  $insert_lead_project = "INSERT INTO `tbl_lead_project`(`lead_project_id`, `project_id`, `lead_id`, `status_id`, 
  `lead_project_description`,
  `managed_by`, `created_by`, 
  `created_on`, `modified_by`, `modified_on`) VALUES ('$uid_lead_project','$project_id','$lead_id','$task_status_id',
  '$task_description','$task_assigned_to','$tasks_logged',
  now(),'$tasks_logged',now())";
//   echo $insert_lead_project;exit();
  $result_lead_project = $con->query($insert_lead_project);
    if($insert_lead_project > 0){
   $insert_lead_task= "INSERT INTO `tbl_lead_task`(`lead_task_id`, `lead_project_id`, `task_name`, 
   `task_description`, `assigned_to`, `status_id`, `modified_by`, `modified_on`) 
   VALUES ('$uid_task','$uid_lead_project','$task_name','$task_description','$task_assigned_to',
   '$task_status_id','$tasks_logged',now())";
    $result_lead_task = $con->query($insert_lead_task);
